  ...
  return (
    <div className=" bg-dark-1 h-1/2 w-full m-0 p-0">
      <div className="bg-dark-1  w-full mx-auto flex flex-col md:flex-row md:justify-around items-center md:items-start p-10  md:p-0">
          ...
          <ul>
            ...
            <div className="flex ml-20 md:ml-0 gap-6 pb-5">
              ...
              <img
                ...
                alt=""
                className="w-5  h-10 cursor-pointer transform hover:scale-150"
              />
            </div>
          </ul>
        </div>
        {sections.map((section, index) => (
          <div key={index} className="mt-10 md:mt-16 w-40 ">
            <p className="text-white font-semibold text-xl md:line-through  pb-4">
              {section.title}
            </p>
            <ul>
              {section.items.map((item, i) => (
                <li
                  key={i}
                  className="text-light-5 text-md pb-2 font-semibold cursor-pointer md:line-through  hover:underline"
                >
                  {item}
                </li>
              ))}
       ...
       <div>
            <FormNewslatter
                ...
                placeholderTextClass="placeholder:text-light-6 md:line-through"  
      ...
      <div className="p-10 text-center">
        <p className="text-white md:text-base font-bold ">
          &copy; 2024 Spine ALL RIGHTS REVERSED
        </p>
      </div>
    </div>
  );
